module Ising_2D_wolff
    ...
    function run_MC(lat::lattice,p::run_parameters,corr_check::Bool)
        ...
        for i in 1:p.N_sweeps
            ...
            ## Choose some random spin 
            ## as seed for the cluster to flip
            x, y = rand(1:lat.L,2)

            ## grow cluster to flip
            grow_cluster(lat,x,y)
    ...
    function grow_cluster(lat::lattice,x::Int64,y::Int64)
        #=
            Function to grow the cluster to be flipped

            Paramters:
            lat - struct lattice
            x,y - position of spin to be used as seed
                  for growing cluster
        =#
        
        ##Note down the spin of the seed
        state = lat.config[x,y]
        ##Flip the seed
        lat.config[x,y] = - lat.config[x,y]
        ##Get the neighbors of the seed
        neigh = neighbors(lat.L,x,y)
        
        ##Go through each neighbor 
        for i in 1:size(neigh)[1]
            x_new, y_new = neigh[i]
            if lat.config[x_new,y_new] == state && rand(Float64) <= (1 - exp(-2*lat.beta))
                grow_cluster(lat,x_new,y_new)
            end
        end
    ...
    function neighbors(L::Int64,x::Int64,y::Int64)
        #=
            To determine the neighbors of (x,y) in lattice
        =#
        return [[periodic_boundary(x-1,L),y],
                [periodic_boundary(x+1,L),y],
                [x,periodic_boundary(y-1,L)],
                [x,periodic_boundary(y+1,L)]]
    end
